...
export function IdleSegment({ onClick, onMore }: { onClick?: () => void; onMore?: () => void; }) {
    return (
        <div
        ...
        >
            ...
            {onMore && (
                <button
                    className="di-idle-more"
                    onClick={e => { e.stopPropagation(); onMore(); }}
                    onMouseDown={e => e.stopPropagation()}
                    aria-label="Open Hub"
                    title="Open Hub (or right-click anywhere)"
                >
...
export function VoiceSegment({ event, expanded, onMouseEnter }: {
    ...
    // Derive title/subtitle live from the stores so channel renames (or the
    // guild name changing) reflect in the pill immediately, without needing a
    // fresh push from liveActivities.
    const liveTitle = useStateFromStores([ChannelStore], () => {
        if (!event.channelId) return event.title;
        const ch = ChannelStore.getChannel(event.channelId);
        if (!ch) return event.title;
        if (ch.name) return `#${ch.name}`;
        if (ch.type === 1) return "DM";
        if (ch.type === 3) return ch.name || "Group DM";
        return event.title;
    });
    const liveSubtitle = useStateFromStores([GuildStore], () => {
        if (!event.guildId) return event.subtitle;
        return GuildStore.getGuild(event.guildId)?.name ?? event.subtitle;
    });
        ...
        >
            <div className="di-seg-compact">
                ...
                <div className="di-seg-text">
                    <Marquee className="di-seg-title">{liveTitle}</Marquee>
                    {liveSubtitle && <span className="di-seg-subtitle">{liveSubtitle}</span>}
                </div>
